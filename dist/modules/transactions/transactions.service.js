...
class TransactionsService {
    ...
    async create(userId, input) {
        return (0, database_1.withTransaction)(async (client) => {
            ...
            if (input.type === 'expense') {
                if (!(0, money_1.canDeduct)(currentBalance, input.amountPaise)) {
                    throw new errors_1.InsufficientBalanceError(`Insufficient balance. Available: ${currentBalance}, Required: ${input.amountPaise}`);
                }
            }
            // Create the transaction
            const { rows: [txn] } = await client.query(`INSERT INTO transactions 
         ...
         RETURNING *`, [
            ...
            await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise + $1, updated_at = NOW() WHERE id = $2', [balanceChange, input.accountId]);
     ...
     */
    async update(userId, txnId, input) {
        return (0, database_1.withTransaction)(async (client) => {
            // Get old transaction
            const { rows: oldTxns } = await client.query('SELECT * FROM transactions WHERE id = $1 AND user_id = $2 FOR UPDATE', [txnId, userId]);
            if (oldTxns.length === 0)
            ...
            const { rows: accounts } = await client.query('SELECT * FROM accounts WHERE id = ANY($1) AND user_id = $2 AND is_active = true FOR UPDATE', [accountIds, userId]);
            if (accounts.length !== accountIds.length) {
                throw new errors_1.NotFoundError('One or more accounts not found.');
            }
            ...
            if (oldTxn.type === 'income') {
                ...
                await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise - $1, updated_at = NOW() WHERE id = $2', [oldBalance, oldAccountId]);
            }
            ...
            if (newType === 'expense') {
                const { rows: [acct] } = await client.query('SELECT current_balance_paise FROM accounts WHERE id = $1', [newAccountId]);
                if (!(0, money_1.canDeduct)(Number(acct.current_balance_paise), newAmount)) {
                    // Rollback will handle reverting step 1
                    throw new errors_1.InsufficientBalanceError(`Insufficient balance after recalculation.`);
                }
            }
            // Step 4: Apply new transaction effect
            const newBalanceChange = newType === 'income' ? newAmount : -newAmount;
            await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise + $1, updated_at = NOW() WHERE id = $2', [newBalanceChange, newAccountId]);
            ...
            if (sets.length > 0) {
                ...
                const { rows } = await client.query(`UPDATE transactions SET ${sets.join(', ')}, updated_at = NOW()
     ...
     */
    async delete(userId, txnId) {
        return (0, database_1.withTransaction)(async (client) => {
            // Lock the transaction
            const { rows: txns } = await client.query('SELECT * FROM transactions WHERE id = $1 AND user_id = $2 FOR UPDATE', [txnId, userId]);
            if (txns.length === 0)
                throw new errors_1.NotFoundError('Transaction not found.');
            const txn = txns[0];
            // Lock the account
            ...
            if (txn.type === 'income') {
                // Was added → subtract
                await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise - $1, updated_at = NOW() WHERE id = $2', [amount, txn.account_id]);
            }
                ...
                await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise + $1 WHERE id = $2', [amount, txn.account_id]);
                await client.query('UPDATE accounts SET current_balance_paise = current_balance_paise - $1 WHERE id = $2', [amount, txn.transfer_to_account_id]);
            }
     ...
     */
    async getById(userId, txnId) {
        const { rows } = await (0, database_1.query)('SELECT * FROM transactions WHERE id = $1 AND user_id = $2', [txnId, userId]);
        if (rows.length === 0)
            throw new errors_1.NotFoundError('Transaction not found.');
        return this.formatTransaction(rows[0]);
    }
    ...
    async list(userId, filters) {
        ...
        // Count total
        const { rows: countRows } = await (0, database_1.query)(`SELECT COUNT(*) AS total FROM transactions t ${whereClause}`, params);
        ...
        const { rows } = await (0, database_1.query)(`SELECT t.*, a.account_name, a.type AS account_type
       FROM transactions t
       LEFT JOIN accounts a ON t.account_id = a.id
       ${whereClause}
       ORDER BY ${sortCol} ${filters.sortOrder}, t.created_at DESC
       LIMIT $${paramIdx++} OFFSET $${paramIdx++}`, [...params, filters.limit, offset]);
        const meta = (0, response_1.buildPaginationMeta)(total, filters.page, filters.limit);
        return {
            transactions: rows.map(this.formatTransaction),
            meta,
        };
    }
    async exportPdf(userId, res) {
        const { rows: [user] } = await (0, database_1.query)('SELECT email, full_name, dob FROM user_profiles WHERE id = $1', [userId]);
        if (!user)
        ...
        const { rows: txns } = await (0, database_1.query)(`SELECT t.*, a.account_name 
       FROM transactions t
       LEFT JOIN accounts a ON t.account_id = a.id
       WHERE t.user_id = $1
       ORDER BY t.transaction_date DESC, t.created_at DESC`, [userId]);
        ...
        const addFooter = () => {
            doc.fontSize(9)
                .fillColor('#94a3b8')
                .text('© All copyrights are reserved by PixoraLabz.tech', 50, doc.page.height - 50, { align: 'center', width: doc.page.width - 100 });
        };
        // --- Header ---
        ...
        if (user.full_name)
            doc.fillColor('#64748b').text(user.email, 50, 145);
